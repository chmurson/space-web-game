...
- The FPS meter graph now scales its Y axis to the maximum frame time in the visible graph window so small frame-time changes remain visible.
...
After that fix, allocation sampling showed debug/UI surfaces as the next visible source when the FPS meter was open. The menu and touch-hint changes remove hidden no-op renders, and the FPS meter/probe changes reduce self-inflicted debug overlay churn and noise. The FPS meter also keeps short frame-time spikes readable by exposing the graph-window max value and scaling the graph to that visible range.
...
- WebKit FPS-meter dynamic-scale check inspected: `tmp/perf/fps-meter-dynamic-scale-webkit.png`; graph path reached the top of the graph at the visible max frame time while the popup remained readable.
