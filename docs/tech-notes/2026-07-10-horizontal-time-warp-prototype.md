...
- `npm run test:gui` passed on the final application behavior: 53 mobile Chromium GUI tests, including resting and held-drag Time Warp screenshots.
...
- Inspected `tmp/playwright-results/mobileHudScreenshot-captur-51097--touch-control-after-reveal-mobile-chromium/mobile-time-warp-control.png` and `mobile-time-warp-control-dragging.png`; both controls are visible, readable, upright, and unobstructed. In the held midpoint state the two equally close labels share the same intermediate emphasis and remain separated, while farther labels progressively recede.
