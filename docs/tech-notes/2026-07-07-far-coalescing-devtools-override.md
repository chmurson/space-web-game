...
- Semantic refreshes still bypass coalescing: initial/manual refresh, target change, horizon change, sampling change, assist change, and control change.
...
- Confirmed `dist/space-web-game-devtools-version.json` reports `{"extensionVersion":"0.1.21"}`.
