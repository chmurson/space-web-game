...
results in a dedicated `review-submissions/` automation-state namespace. Before
delegating a formal-review finding, triage re-fetches the exact review and
compares its repository, PR, review id, body hash, `submitted_at`, `commit_id`,
and `state` with the stored addressed fingerprint.
