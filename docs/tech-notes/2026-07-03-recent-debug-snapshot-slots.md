...
- `tests/debugScenarioSnapshot.test.ts` covers list capacity, ordering, generated labels, storage scope, and selected-load behavior.
- `tests/gui/mobileHudScreenshot.spec.ts` covers the top-menu picker adapter path.

## Decisions

- Recent entries are current-session only. A follow-up review found that persisting full snapshots duplicated the active snapshot payload in localStorage and increased quota risk; the existing active slot remains the durable browser save path.
...
- The top-menu and main-menu recent pickers still use separate small render blocks; extracting a shared component was not worth the PR churn for two simple call sites.
