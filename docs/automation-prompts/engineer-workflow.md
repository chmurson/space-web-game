...
10. After delegation, before yielding while the worker is active, persist the worker id, claim/token path, sidecars, and next reconciliation action in the claim/sidecar and scheduler/worker continuation records, then atomically prepend a corresponding event to automation memory without changing older entries. Do not create or rewrite a mutable active-handoff/current-state memory section. Keep the orchestrator task unfinished while the worker is active. Wait for a terminal result or create a continuation/wakeup handoff; the scheduler/runtime must resume this same task to reconcile the worker result rather than mark the run complete.
...
13. Only after delegated-worker reconciliation and claim release, atomically prepend the current run time and terminal outcome to automation memory:
    - Read the existing memory before writing, add one compact newest-first event, and preserve all prior entries byte-for-byte.
