...
- Regression guard: an actionable human-authored `issue_comment` that directly mentions `@andrzejkoduje` and contains an unchecked concrete checklist must be surfaced even when all inline review threads are resolved and CodeRabbit has no actionable finding. This guard is about future triage; do not assume or implement the product request while validating the workflow.
...
- Only when the recorded worker returns a terminal result while its claim is still valid may the same-automation invocation reconcile the worker handoff and triggering sidecars. If the worker is terminal, perform the terminal-result reconciliation immediately, even when its claim is still fresh; terminal worker status overrides the freshness wait. Once the freshness window has elapsed, classify unavailable worker state as `uncertain` until direct worker or claim-helper evidence resolves it.
- A terminal worker result means worker execution ended, not that the handoff is complete. Re-fetch every triggering comment from its original source and its reactions before any new delegation. If an unchanged comment already has `rocket`, mark its sidecar `addressed` and skip the worker-result/delegation path for that comment. Otherwise compare the current `updatedAt` and body hash with the sidecar, and independently map every extracted request/checklist item using the current comment and resulting PR/GitHub state. Use the worker report as evidence, but do not inherit its completion claim without verification.
...
- After every triggering sidecar is reconciled, release the task claim, then archive the matching durable handoff with outcome `reconciled` or `abandoned`. If either action fails, keep the record pending and report the handoff as unfinished. If reconciliation produces more delegated work, create a new handoff only after the prior one was archived and repeat this boundary. Only after reconciliation, claim release, and handoff archive may the orchestrator prepend a terminal memory event and emit its final report.
...
2. Resolve the active automation id and list pending durable handoffs. Reconcile every same-automation handoff before new triage; verify each matching claim and retain `awaiting_worker` or `awaiting_status` records without starting another task.
3. Read automation memory as historical event context, then derive current state from live GitHub/`gh`, claim, sidecar, durable handoff, worker-status, and branch/worktree state before deciding what is active or actionable.
4. For every automation-owned open PR, independently fetch all issue-level PR comments and all inline review comments, fetch review-thread state, and build the classified inventory. Record source counts and failures; do not advance if either required source is incomplete.
5. Apply the priority order to the classified inventories. Claim only the first actionable task this run can safely own.
6. For a PR candidate, acquire `pr` claim with branch, record one sidecar per triggering comment, add `eyes` to each exact comment, then delegate using the Worker Prompt Template.
...
11. After delegation, before yielding while the worker is active, create the durable handoff record with worker id, parent id, scope, claim/token path, sidecars, worktree, and next reconciliation action. Then use the dedicated short-lived memory-write protocol above to prepend a corresponding event without changing older entries. Do not create or rewrite a mutable active-handoff/current-state memory section. A later scheduled invocation with the same automation id must reconcile the record; it need not resume the original parent task.
12. After a terminal worker result, re-fetch every triggering comment from its original source, compare `updatedAt` and body hash, and independently map every request item. Add `rocket` and mark only matching sidecars `addressed` when the comment is unchanged, every extracted request/checklist item is independently mapped to `addressed` or evidenced `not_applicable`, and no item remains `not_addressed`; otherwise add no `rocket` and record stale/partial/blocker/failure evidence. Release the claim only after all sidecars are reconciled or intentional abandonment is recorded, then archive the handoff.
