...
- Add `rocket` to an exact triggering comment only when that comment is unchanged and every extracted request item is independently mapped to `addressed`. Otherwise add no `rocket`; update its sidecar to `stale`, `partial`, `blocked`, or `failed` with the current version, per-item mapping, evidence, and reconciliation timestamp.
