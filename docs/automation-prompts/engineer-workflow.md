...
- Derive current state at decision time from fresh live GitHub/`gh` data, claim and sidecar files, direct delegated-worker status, and current branch/worktree Git state. When those sources disagree with memory, the live sources win and the discrepancy should be recorded as a new memory event.
- Claim-first reconciliation: the live task claim is the concurrency authority. Before interpreting memory, worker ids, continuation records, or sidecars, read the matching claim. If it is `active`, unexpired, and `last_seen` is within the 2-hour reconciliation freshness window, treat the task as in progress, regardless of whether worker metadata is missing, stale, or temporarily unavailable. Do not release, replace, or duplicate that claim. If this is a foreign claim, skip that task and continue with other claimable work; if it is owned by this run, wait for the owning worker/continuation or the normal claim-expiry path.
- Sidecars, memory events, and worker ids are audit and wakeup hints, not ownership state. They may explain what to reconcile after the claim is released, but they must never override a recent active claim or justify a second worker.
...
- On every scheduler wakeup or new workflow invocation, inspect live claims before possible active-worker handoffs or fresh PR/issue triage. An unexpired `active` claim whose `last_seen` is within the 2-hour reconciliation freshness window is sufficient evidence that the task is still owned. For a foreign claim, skip that task and continue triage; for a claim owned by this run, continue waiting or refresh the continuation. Once the freshness window has elapsed, question the handoff using worker status, sidecars, and branch/worktree state, but do not replace or release the claim unless the claim helper's normal TTL and PID-liveness rules permit it.
- Do not reconcile persisted active-worker handoffs before performing fresh PR/issue triage for a foreign recent claim; skip that task and consider other work. For a claim owned by this run, resume the handoff rather than starting another task.
- Only when the claim is no longer active, or the owning worker returns a terminal result while its claim is still valid, reconcile the worker handoff, triggering comment sidecars, and dedicated review-submission records. If worker status is unavailable while the claim is within the freshness window, do not classify the work as terminal or failed. After the freshness window, classify the state as `uncertain` until direct worker or claim-helper evidence resolves it.
- If the worker is terminal, perform the terminal-result reconciliation immediately, even when its claim is still fresh; terminal worker status overrides the freshness wait for the owning run.
- If the run must yield while a claim is recent, preserve the active delegated-worker, claim, and continuation/wakeup handoff state for the same task, including the next reconciliation action; the claim remains authoritative until the worker or normal expiry path changes it.
- While waiting on that claim, continue the existing handoff rather than starting another task.
- An orchestrator task that delegated work remains unfinished while any delegated worker is active. Do not emit the final report, prepend a terminal memory event, release the task claim, or clean up that task’s branch/worktree while its worker is still active.
- Wait for a terminal worker result. If the current invocation cannot remain attached, use a continuation/wakeup handoff for the same unfinished orchestrator task. Preserve the worker/run id, claim and token path, triggering comment sidecars and dedicated review-submission records, observed comment/review versions and item inventory, and the next reconciliation action in the operational handoff records, then prepend a matching historical memory event. A yield or handoff is not run completion, and the claim must remain live until reconciliation resumes.
...
10. After delegation, before yielding while the worker is active, persist the worker id, claim/token path, triggering comment sidecars, dedicated review-submission records, and next reconciliation action in the claim/sidecar and scheduler/worker continuation records, then use the dedicated short-lived memory-write protocol above to prepend a corresponding event without changing older entries. Do not create or rewrite a mutable active-handoff/current-state memory section. Keep the orchestrator task unfinished while the worker is active. Wait for a terminal result or create a continuation/wakeup handoff; the scheduler/runtime must resume this same task to reconcile the worker result rather than mark the run complete.
11. After a terminal worker result, re-fetch every triggering record from its original source and independently map every request item. Reconcile comments through their `updatedAt`/body hash and reaction rules. Reconcile review submissions through their complete body/version fingerprint without requiring a reaction. Release the claim only after every triggering comment sidecar and dedicated review-submission record is durably reconciled, or intentional abandonment is recorded.
12. Perform conservative cleanup only when safety is unambiguous.
13. Only after delegated-worker reconciliation and claim release, prepend the current run time and terminal outcome using the dedicated short-lived memory-write protocol:
