...
- On every scheduler wakeup or new workflow invocation, reconcile possible active-worker handoffs before performing fresh PR/issue triage. Use the newest memory events only to locate candidate handoffs, then derive their current state from direct worker status, live claims and sidecars, live GitHub/`gh`, and branch/worktree state. If a worker is terminal, perform the terminal-result reconciliation immediately; if it is still active, continue waiting or create/refresh the continuation rather than starting another task.
