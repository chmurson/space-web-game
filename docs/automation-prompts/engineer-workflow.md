...
- On every scheduler wakeup or new workflow invocation, reconcile persisted active-worker handoffs before performing fresh PR/issue triage. Read the saved worker id, claim/token path, sidecar records, and next reconciliation action; if the worker is terminal, perform the terminal-result reconciliation immediately, and if it is still active, continue waiting or create/refresh the continuation rather than starting another task.
...
10. After delegation, before yielding while the worker is active, atomically read/modify/write automation memory to preserve the active delegated-worker, claim, and continuation/wakeup handoff state, including the next reconciliation action. Keep the orchestrator task unfinished while the worker is active. Wait for a terminal result or create a continuation/wakeup handoff; the scheduler/runtime must resume this same task to reconcile the worker result rather than mark the run complete.
...
13. Only after delegated-worker reconciliation and claim release, atomically read/modify/write the current run time and terminal outcome into automation memory. Preserve the existing memory:
