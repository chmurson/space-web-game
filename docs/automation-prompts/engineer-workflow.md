...
- Persist one record per review submission under the active automation's dedicated `review-submissions/` state directory, named `pr-<number>-review-<review-id>.json`. Follow the common sidecar convention by recording the URL/id, `kind: review_submission`, fingerprint fields, extracted request/checklist items, claim and worker/run fields when applicable, status, per-item reconciliation, evidence, and reconciliation timestamp.
...
- Only when the claim is no longer active, or the owning worker returns a terminal result while its claim is still valid, reconcile the worker handoff and triggering sidecars. If worker status is unavailable while the claim is within the freshness window, do not classify the work as terminal or failed. After the freshness window, classify the state as `uncertain` until direct worker or claim-helper evidence resolves it.
...
- Wait for a terminal worker result. If the current invocation cannot remain attached, use a continuation/wakeup handoff for the same unfinished orchestrator task. Preserve the worker/run id, claim and token path, triggering sidecars, observed comment/review versions and item inventory, and the next reconciliation action in the operational handoff records, then prepend a matching historical memory event. A yield or handoff is not run completion, and the claim must remain live until reconciliation resumes.
