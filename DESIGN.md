...
- Info is a compact telemetry surface, not a second navigation system. Desktop uses a text-only top-bar pill with an anchored popover that stays open until the player closes it, including across outside clicks; do not add a selected-count badge because the readout rail already communicates the current selections. Selected rows become persistent readouts: a bounded vertical rail sits at the right on desktop and at the top right below the safe-area-aware telemetry on mobile. Each rail must shrink-wrap its visible readouts until its maximum height so collision avoidance does not reserve empty screen space. Each visible readout reuses the top telemetry pill surface, sizing, and typography. On mobile, let the readout button hug the natural pill height so the rail remains visually compact and keep it visible while Info is open; the desktop rail remains hidden behind its open popover.
- Info shows the current target first with the same automatic, pinned, or locked target-status icon used by the Target pill and selector. Its row is always checked and immutable while active, uses a recessed fixed-cyan selected treatment instead of the player-pin dot, and does not produce a persistent readout because the top Target pill already exposes the same telemetry. Selected entries follow it, then remaining entries retain their physical-distance ordering from the target body. Each scenario body appears once with the same body glyph used by Target; body values remain spacecraft-to-body physical surface distances and show `to spacecraft`. `Pe` and `Ap` share one selectable row whose state controls both orbit points together, show physical altitudes over the active target, label that target once, and use an unavailable dash when current prediction data does not match the target. Do not add separate spacecraft or duplicate active-target rows, and do not offer `Select all`.
- Stored player selections, scenario selections, and targeting keep distinct ownership. The active-target selection is derived and does not mutate stored pins; when the target changes, each body returns to its stored selection state. Scenario selections are checked, labeled, and immutable; `Clear` and `Shift+I` remove player selections only, so the active target remains selected. Use `I` to toggle the relevant desktop or mobile Info surface.
...
**Top-menu Snapshot export:** Keep export actions inside the existing `Save debug snapshot` and `Load debug snapshot` submenus rather than adding a separate top-level section. Save offers `Save` and `Save & export`; the combined action captures once, saves the named snapshot as the active and recent entry, downloads that same capture, and marks that one recent entry as exported. Load offers `Load` and `Export` for the selected recent entry, with both actions sharing the same enabled state. Export leaves each submenu open, keeps the selection stable, returns focus to the activated export action after rendering, and shows an accessible success or partial/full-failure message directly below the actions. Clear transient export feedback when leaving or closing the submenu, and never describe a started download as a total failure merely because local metadata persistence failed.
...
**HUD and telemetry:** Keep telemetry compact, mostly non-interactive, and readable while the canvas moves. Use icons or short labels before long text. Use ellipsis or visually hidden target text on small screens rather than widening the top bar. The top Target pill owns active-target distance disclosure; other Info selections own persistent numeric readouts. Body readouts reuse the body's colored sphere and show `<sphere> <name> · <surface distance>`; the atomic apsides readout stays text-only and shows `Pe · <altitude> | Ap · <altitude>`. Match the Target pill hierarchy in every readout: object and point names use primary telemetry type, while distance values and their separators use the secondary telemetry treatment even when a value is unavailable (`—`). Player-owned readouts unselect directly; scenario-owned readouts remain visibly identified and immutable.
...
**Touch controls:** Preserve safe-area offsets, large hit areas, panel-owned scrolling, and tutorial focus affordances. Touch-control panels should use shared glass panel values with local accent colors for thrust, time warp, target, and trajectory semantics. Player turning is always direct: mobile uses analog RCS yaw, while desktop uses `A`/`D` or Left/Right Arrow with `Shift` for precise input. Empty-playfield taps and drags never create, preview, confirm, or cancel a target-heading plan; drags remain camera input. Touches, drags, and scrolling that begin inside a command panel remain owned by that panel and must not reach the playfield camera. RCS feedback visualizes the turn actually performed rather than a future heading target.

**Mobile command dock:** Mobile touch gameplay uses the compact four-item dock in `Flight`, `Info`, `Nav`, `Ship` order with standard safe-area spacing and subtle selected-state emphasis. Flight, Info, and Nav are enabled; Ship remains visible but disabled until its panel has real content. Settings are intentionally absent from the mobile dock and mobile in-game controls because there are no player-facing mobile settings. At most one panel is open, selecting the active item collapses it, and selecting another enabled item switches directly. Flight keeps its non-modal, label-free floating layout with analog RCS yaw toward the left safe-area edge and Main Thrust toward the right. A latched Main Thrust burn remains active when Flight collapses or another dock opens; interaction disable, blur, and Flight-control unavailability still turn it off. Info uses the same row and selection-management surface as desktop. Its vertically scrollable persistent readouts stay fixed at the top right below the safe-area-aware telemetry without reserving dock space, use the natural telemetry-pill height for a compact stack, and remain visible while Info itself is open. Nav uses one concrete, vertically scrollable glass panel that never scrolls horizontally. Its wide left column stacks Time Warp above Camera; the narrow right column keeps the vertical Trajectory selector above a current-target button. The target button reports the current body and toggles the existing Target selector as a popup over the left column, leaving the button and Trajectory reachable while the popup is open. Clicking or touching outside the popup closes only the Target selector and leaves Nav open; interaction inside the popup and target commits keep it open. Time Warp remains visually dominant, with a compact full-width horizontal selector where left is faster and right is slower. The selector itself communicates the current rate; duplicate heading values and visible helper/status copy are omitted, while policy feedback remains available to assistive technology. Camera controls keep `Follow` choices for `Spacecraft` or `Target`, and the compact `Recenter` action clears the player's pan offset. On coarse-pointer mobile surfaces, the separate in-game Controls popover omits Camera and Trajectory because Nav is their sole home; fine-pointer desktop retains both controls in that popover. Target selection and automatic return keep Nav and the Target popup open; recommendation, manual, and forced states retain their existing cyan/status semantics. Scenario availability hides only the unavailable Target or Trajectory section and leaves Nav plus unrelated controls stable. Target and Trajectory placement preferences are no longer shown or applied; legacy persisted values remain inert pending storage cleanup. Changing Follow preserves the relative pan offset, just like a target change. Neutral framing always centers the followed object within the current playable viewport above the dock and its open panel; drag and edge pan add a relative offset without introducing a player-visible lock mode. Closing, switching, Escape, interaction disable, blur, or control unavailability clears the outgoing owned gesture. Tutorial burn and RCS focus open Flight; tutorial warp, Target, and Trajectory focus opens Nav and highlights the relevant control, with Target focus also opening its popup.

**Body labels and offscreen indicators:** Body labels contain only the body name. While onscreen, show a label for three seconds after viewport entry or while the body's apparent radius is small; enter the persistent small-body state at `6px` apparent radius, retain it through `8px`, and hide it above that threshold. Selection and active-target state do not change label content, style, duration, or visibility. Every body offscreen arrow is hollow, unlabeled, and independent of Info selection; the active target receives the stronger cyan stroke/glow while remaining hollow. The spacecraft arrow is solid bright white, visually unlabeled, accessible as `Spacecraft, off screen`, and keeps placement priority over body indicators. Body labels and compact `Pe`/`Ap` markers are selection controls, while direct canvas raycasts are limited to visible bodies so empty-canvas camera gestures stay available.

**Experimental sphere-of-influence overlays:** Keep gravitational sphere-of-influence visuals hidden in the default experience. All exact `soi=1` through `soi=4` URL flags use the selected former `soi=2` soft field, the current former-`soi=5` constant-perceived-width edge gradient, and the same 1 pixel screen-space border. They remain visually identical through wide and middle zoom, then compare only how thin the body-colored edge-gradient wall becomes in the final decade of local zoom: at maximum zoom-in, `soi=1`, `soi=2`, `soi=3`, and `soi=4` retain 25%, 15%, 10%, and 5% of the former `soi=5` wall width. Keep the true border screen-space sized so its apparent width remains uniform at every zoom. Render the body-colored field as a cross-section in the orbital plane so it remains readable in the planar simulation and below bodies, trajectories, labels, and HUD. The overlay follows the body without inheriting its visual spin. Do not add a permanent HUD control, legend, or label while the feature remains an experiment.

**Orbit-point markers:** Keep periapsis and apoapsis as compact cyan inverted-house markers whose tips identify the orbit location and whose faces contain only `Pe`/`Ap`; do not add a second Three.js dot or shape beneath them. Both markers remain hidden until the combined Pe/Ap Info row is selected, then each available prediction event may show under the existing zoom, prediction-availability, and tutorial constraints. Do not add a separate marker-visibility setting, native tooltip, or CSS-generated numeric tooltip; the combined Pe/Ap Info selection owns marker visibility, while numeric altitude belongs in Info and the combined persistent readout. Direct marker activation toggles that combined selection.
