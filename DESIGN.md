...
**Experimental sphere-of-influence overlays:** Keep gravitational sphere-of-influence visuals hidden in the default experience. The exact `soi=1`, `soi=2`, `soi=3`, and `soi=4` URL flags expose the same approved soft-field gradient with a 1, 2, 3, or 4 pixel border respectively. Keep the border screen-space sized so its apparent width remains uniform at every zoom. Render the body-colored field as a cross-section in the orbital plane so it remains readable in the planar simulation and below bodies, trajectories, labels, and HUD. The overlay follows the body without inheriting its visual spin. Do not add a permanent HUD control, legend, or label while the feature remains an experiment.
