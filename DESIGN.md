...
**Experimental sphere-of-influence overlays:** Keep gravitational sphere-of-influence visuals hidden in the default experience. All exact `soi=1` through `soi=5` URL flags use the selected former `soi=2` soft field and its 1 pixel screen-space border. They compare only how strongly zoom affects the perceived width of the body-colored gradient close to the SOI edge: `soi=1` keeps the current world-space growth, while `soi=2`, `soi=3`, and `soi=4` reduce that growth by 25%, 50%, and 75%; `soi=5` keeps the perceived gradient width constant. Keep the border screen-space sized so its apparent width remains uniform at every zoom. Render the body-colored field as a cross-section in the orbital plane so it remains readable in the planar simulation and below bodies, trajectories, labels, and HUD. The overlay follows the body without inheriting its visual spin. Do not add a permanent HUD control, legend, or label while the feature remains an experiment.
