...
**Top-menu Snapshot export:** Keep Snapshot as a distinct labeled section on the main top-menu surface. Its `Export current state` action captures the live runtime state when pressed and starts a JSON download. Keep the menu open and return focus to the export action after rendering a result. Show the transient result directly below the action: `Current state download started.` as a success status, `Download started, but the recent snapshot could not be saved.` as an error alert for partial success, or `Snapshot download could not be started. Try again.` as an error alert for full failure. Clear the message when the menu closes, and never describe a started download as a total failure merely because recent-entry persistence failed.
