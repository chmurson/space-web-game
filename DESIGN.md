...
**Experimental sphere-of-influence overlays:** Keep gravitational sphere-of-influence visuals hidden in the default experience. The exact `soi=1` URL flag exposes the approved soft-field base with its 1 pixel screen-space border. The exact `soi=2`, `soi=3`, `soi=4`, and `soi=5` flags keep that construction and progressively raise only the body-colored gradient strength close to the SOI edge to 1.5×, 2×, 2.5×, and 3× the base. Keep the border screen-space sized so its apparent width remains uniform at every zoom. Render the body-colored field as a cross-section in the orbital plane so it remains readable in the planar simulation and below bodies, trajectories, labels, and HUD. The overlay follows the body without inheriting its visual spin. Do not add a permanent HUD control, legend, or label while the feature remains an experiment.
