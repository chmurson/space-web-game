...
**Target and offscreen indicators:** Keep labels small, wrapped on mobile, and attached to the playfield. Use cyan as the default navigational cue. An unselected active target has no visible tooltip and uses the same unlabeled hollow offscreen arrow as another unselected body. Selecting the target shows its stable name plus spacecraft-to-surface distance without the transient target-name reveal and gives its offscreen indicator the emphasized target treatment. Other selected offscreen bodies show name plus physical spacecraft-to-surface distance. The offscreen spacecraft indicator is solid, named `Spacecraft`, contains no numeric distance, and takes placement priority over body indicators. Body labels and compact `Pe`/`Ap` markers are button-sized selection controls, but pointer handling remains limited to those controls and direct canvas hits on their matching world objects so empty-canvas maneuver and camera gestures stay available.

**Orbit-point markers:** Keep periapsis and apoapsis as visually distinguishable cyan/amber inverted-house markers whose tips identify the orbit location and whose faces contain only `Pe`/`Ap`. Selecting either point in Info reveals its separate altitude tooltip; the marker itself never includes a numeric distance. Settings expose marker visibility as the sole orbit-point display preference, and direct marker activation toggles the corresponding Info selection.
