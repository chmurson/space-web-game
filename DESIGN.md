...
**Target and offscreen indicators:** Keep labels small, wrapped on mobile, and attached to the playfield. Use cyan as the default navigational cue. A visible non-target body uses its name only. The active target is visually distinct and shows its name plus spacecraft-to-surface distance for three seconds after a target change, then distance only; its accessible label always retains both values. Off-screen active or pinned bodies show name plus physical spacecraft-to-surface distance, while an unpinned non-target body becomes an unlabeled hollow arrow. The off-screen spacecraft indicator is solid, named `Spacecraft`, contains no numeric distance, and takes placement priority over body indicators. Body labels and the compact `Pe`/`Ap` labels are button-sized pin controls, but pointer handling remains limited to those controls and direct canvas hits on their matching world objects so empty-canvas manoeuvre and camera gestures stay available.

**Orbit-point markers:** Keep periapsis and apoapsis as visually distinguishable compact glyphs with optional short `Pe`/`Ap` labels only; do not place numeric orbit-point values on the playfield. The Info surface owns their numeric target-surface altitudes. Settings expose marker visibility as the sole orbit-point display preference, and the marker or label can toggle the corresponding Info pin.
