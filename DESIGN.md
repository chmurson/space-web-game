...
- Info is a compact telemetry surface, not a second navigation system. Desktop uses a top-bar pill with an anchored popover that stays open until the player closes it, including across outside clicks. Mobile uses the Info dock panel without a separate persistent pin rail.
- Info shows the current target first, selected entries next, and remaining entries by physical distance from the target body. Each scenario body appears once with the same body glyph used by Target; body values remain spacecraft-to-body physical surface distances and show `to spacecraft`. `Pe` and `Ap` share one selectable row whose state controls both orbit points together, show physical altitudes over the active target, label that target once, and use an unavailable dash when current prediction data does not match the target. Do not add separate spacecraft or duplicate active-target rows.
- Player selections and scenario selections keep distinct ownership. Scenario selections are checked, labeled, and immutable; `Clear` and `Shift+I` remove player selections only. Use `I` to toggle the relevant desktop or mobile Info surface.
...
**Target and offscreen indicators:** Keep labels small, wrapped on mobile, and attached to the playfield. Use cyan as the default navigational cue. An unselected active target follows the same name-only visibility rule as another unselected body and uses the same unlabeled hollow offscreen arrow. Selecting the target shows its stable name plus spacecraft-to-surface distance and gives its offscreen indicator the emphasized target treatment. Other selected offscreen bodies show name plus physical spacecraft-to-surface distance. The offscreen spacecraft indicator is solid, named `Spacecraft`, contains no numeric distance, and takes placement priority over body indicators. Body labels and compact `Pe`/`Ap` markers are selection controls, while direct canvas raycasts are limited to visible bodies so empty-canvas maneuver and camera gestures stay available.

**Orbit-point markers:** Keep periapsis and apoapsis as compact cyan inverted-house markers whose tips identify the orbit location and whose faces contain only `Pe`/`Ap`; do not add a second Three.js dot or shape beneath them. Both markers remain hidden until the combined Pe/Ap Info row is selected, then each shows its altitude tooltip. The marker face itself never includes a numeric distance. Settings expose marker visibility as the sole orbit-point display preference, and direct marker activation toggles the combined Pe/Ap Info selection.
