...
- Iteration: move the center dot out of the transient ripple so the ring still fades on its short timer, but the dot remains until target-heading feedback ends.
...
- [x] Keep the target-heading dot visible until target-heading feedback ends.
...
Behavior implemented: Double-click target markers still appear transiently and follow the selected world position, but now use a single thin ring instead of bright expanding circles or crosshair ticks. The ring still expands and fades on the short ripple timer. The center dot is now part of the target-heading feedback, so it remains at the selected point until the ship completes the turn and the target-heading UI disappears. The target-heading circular arc is replaced with a translucent annular slice between the ship's current heading and target heading. The slice has no border or glow. The straight target-heading line remains dashed and supporting, with slightly higher opacity after the latest tuning pass.
...
- `npm run build` passed after the initial implementation, after simplifying the visuals, after increasing animation opacity, after keeping the center dot steady, and after tying the dot to target-heading feedback.
- `git diff --check` passed after the initial implementation, after simplifying the visuals, after increasing animation opacity, after keeping the center dot steady, and after tying the dot to target-heading feedback.
- `npm run deploy:netlify` passed after the initial implementation, after simplifying the visuals, after increasing animation opacity, after keeping the center dot steady, and after tying the dot to target-heading feedback. Latest staging URL: https://fanciful-bunny-d77b4b.netlify.app. Latest unique deploy URL: https://6a3245d169a284d98248b722--fanciful-bunny-d77b4b.netlify.app.
