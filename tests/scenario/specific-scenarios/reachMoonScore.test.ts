...
describe('reachMoonScore', () => {
  ...
  it('matches the fuel-left target curve from the scoring spreadsheet', () => {
    const fuelTargets = [
      [0, 0],
      [0.1, 10.2],
      [0.2, 20.1],
    ...
    for (const [fuelRemainingRatio, fuelBonusPoints] of fuelTargets) {
      expect(
        calculateReachMoonScore({
          fuelCapacityKg: 32_000,
          fuelRemainingRatio,
  ...
  it('matches the elapsed-day target curve from the scoring spreadsheet', () => {
    const dayTargets = [
      [1, 49.9],
      [2, 45],
      [3, 40.1],
    ...
    for (const [elapsedDays, timePenaltyPoints] of dayTargets) {
      expect(
        calculateReachMoonScore({
          fuelCapacityKg: 32_000,
