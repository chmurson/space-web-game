...
import { shouldCaptureBurnForMetrics, shouldCircularizeBurn } from "../assist/orbitalAssist";
...
import { formatBodyInfluences, formatDistance, formatDuration, formatSpecificEnergy } from "./formatters";
...
const getCaptureGuidanceText = ({
  ...
  if (assistMode === "circularize") {
    ...
    return shouldCircularizeBurn(targetMetrics, circularizePlan) ? `Circularizing ${circularizePlan.deltaV.toFixed(0)} m/s` : "Orbit stable-ish";
  }
...
export const getDebugPanelLines = (input: DebugPanelTextInput) => {
  const lines = [
    ...
    input.predictedTargetClosestApproach
      ...
      : "pred Pe: calculating",
    input.predictedImpact ? `pred impact: ${input.predictedImpact.bodyName} in ${formatDuration(input.predictedImpact.time)}` : "pred impact: none",
  ];

  if (input.performanceDebugEnabled) {
    const frameBudgetMs60 = 1000 / 60;
    const frameBudgetMs30 = 1000 / 30;
    const limitingFrameMs = input.smoothedGpuMs === null ? input.smoothedCpuMs : Math.max(input.smoothedCpuMs, input.smoothedGpuMs);
    lines.push(
