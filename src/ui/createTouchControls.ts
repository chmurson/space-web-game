...
export const createTouchControls = (options: {
  ...
  const getTouchDistance = (first: Touch, second: Touch) => Math.hypot(first.clientX - second.clientX, first.clientY - second.clientY);
  const suppressDefaultTouchBehavior = (event: TouchEvent) => {
    event.preventDefault();
  };

  panel.addEventListener("touchstart", suppressDefaultTouchBehavior, { passive: false });
  panel.addEventListener("touchmove", suppressDefaultTouchBehavior, { passive: false });
  panel.addEventListener("touchend", suppressDefaultTouchBehavior, { passive: false });
  panel.addEventListener("touchcancel", suppressDefaultTouchBehavior, { passive: false });
    ...
    (event) => {
      ...
      for (const touch of Array.from(event.changedTouches)) {
        const isDoubleTapCandidate =
          ...
          Math.hypot(touch.clientX - lastTap.x, touch.clientY - lastTap.y) <= tapMoveTolerancePx * 2;
  ...
  panel.addEventListener(
    ...
    (event) => {
      ...
      const factor = clamp(twoFingerGesture.lastDistance / distance, pinchZoomMinFactor, pinchZoomMaxFactor);
  ...
  panel.addEventListener(
    ...
    (event) => {
      ...
      const touch = Array.from(event.touches).find((candidate) => candidate.identifier === doubleTapZoom?.touchId);
      if (!touch) {
        return;
      }
    ...
    (event) => {
      ...
      for (const touch of Array.from(event.changedTouches)) {
        ...
        const isTap =
          ...
          Math.hypot(touch.clientX - tapState.startX, touch.clientY - tapState.startY) <= tapMoveTolerancePx;
  ...
  panel.addEventListener(
    ...
    (event) => {
      ...
      cancelAllManualGestures();
      if (doubleTapZoom && Array.from(event.changedTouches).some((touch) => touch.identifier === doubleTapZoom?.touchId)) {
        doubleTapZoom = null;
      }
      for (const touch of Array.from(event.changedTouches)) {
        tapTouches.delete(touch.identifier);
  ...
  panel.addEventListener("pointermove", (event) => {
    ...
    const isHorizontalDominant = Math.abs(totalDeltaX) > Math.abs(totalDeltaY) * 1.15;
    const isVerticalDominant = Math.abs(totalDeltaY) > Math.abs(totalDeltaX) * 1.15;

    if (!state.thrustActive && Math.hypot(event.clientX - state.startX, event.clientY - state.startY) > tapMoveTolerancePx) {
      clearHoldTimer(state);
    }
    ...
    const steps = Math.trunc(state.verticalCarry / singleFingerWarpStepDistancePx);
    if (steps === 0) {
      return;
    }
