...
const syncDashedLineEndpoint = (material: LineMaterial, lineDistance: number) => {
  ...
  material.dashOffset = ((endpointPhase - lineDistance) % dashPeriod + dashPeriod) % dashPeriod;
};
...
export const updateLine2Geometry = (
  ...
  const nextGeometry = options.replaceGeometryOnUpdate ? new LineGeometry() : geometry;
