...
const getSnapshotCoastPredictionHorizonHours = (snapshot: DebugScenarioSnapshot) =>
  ...
  ("coastPredictionHorizonMultiplier" in snapshot && snapshot.coastPredictionHorizonMultiplier
    ? snapshot.coastPredictionHorizonMultiplier * 4
    : undefined);

const getSnapshotScenarioSession = (snapshot: DebugScenarioSnapshot): RuntimeScenarioSession =>
...
export const createSnapshotFromState = (
  ...
  runtimeScenario: options.scenarioSession ? cloneRuntimeScenarioSession(options.scenarioSession) : undefined,
...
export const readDebugScenarioSnapshot = (): DebugScenarioSnapshot | null => {
  try {
    ...
    return (snapshot.version === 1 || snapshot.version === 2) && Array.isArray(snapshot.bodies) && snapshot.spacecraft ? snapshot : null;
  } catch {
    return null;
  }
};
