...
const gravityAt = (position: Vec2, bodies: Body[], ignoredBodyId?: string): Vec2 => {
  ...
  for (const body of bodies) {
    ...
    acceleration = add(acceleration, scale(offset, accelerationMagnitude / distance));
  }
...
const spacecraftThrustAcceleration = (spacecraft: Spacecraft, state: SimulationState, dt: number) => {
  ...
  const forwardForce = MAIN_THRUST * controls.main - REVERSE_THRUST * controls.reverse;
  const strafeForce = STRAFE_THRUST * controls.strafe;

  return {
    acceleration: add(scale(forward, (forwardForce / mass) * fuelRatio), scale(right, (strafeForce / mass) * fuelRatio)),
...
export const semiImplicitEuler: PhysicsEngine = {
  ...
  step(state, dt) {
    ...
    const velocity = add(state.spacecraft.velocity, scale(add(gravity, thrust.acceleration), dt));
