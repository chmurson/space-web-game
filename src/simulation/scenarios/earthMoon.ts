...
export const createMoonCaptureDebugScenario = (): Scenario => {
  ...
  const approachDistance = Math.sqrt(approachOffset.x ** 2 + approachOffset.y ** 2);
  const towardMoon = {
  ...
  return {
    ...
    description: "Start near the Moon capture-assist range with an incoming trajectory.",
    spacecraft: {
      ...scenario.spacecraft,
      position: {
