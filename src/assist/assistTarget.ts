...
const getTrajectoryCenter = (
	simulationState: SimulationState,
	predictedTrajectoryEnd: Vec2 | null,
): Vec2 | null =>
	predictedTrajectoryEnd
		? scale(
				add(simulationState.spacecraft.position, predictedTrajectoryEnd),
				0.5,
			)
		: null;

const getTrajectorySegmentWeightedCenter = (
	predictedTrajectoryPoints: Vec2[],
): Vec2 | null => {
	...
	for (
		let index = 0;
		index < predictedTrajectoryPoints.length - 1;
		index += 1
	) {
		...
		weightedMidpointSum = add(
			weightedMidpointSum,
			scale(segmentMidpoint, segmentLength),
		);
		totalSegmentLength += segmentLength;
	}

	return totalSegmentLength > 0
		? scale(weightedMidpointSum, 1 / totalSegmentLength)
		: null;
};
...
const getNearestSurfaceDecision = (
	...
	const trajectoryCenter =
		getTrajectorySegmentWeightedCenter(predictedTrajectoryPoints) ??
		getTrajectoryCenter(simulationState, predictedTrajectoryEnd);
	const referencePoint =
		trajectoryCenter ?? simulationState.spacecraft.position;
	...
	if (currentMetric && currentMetric.body.id !== bestMetric.body.id) {
		target =
			bestMetric.midpointDistance * autoSelectConfig.switchRangeMultiplier <
			currentMetric.midpointDistance
				? bestMetric.body
				: currentMetric.body;
	}
		...
		debug: {
			...
			bodies: metrics.map((metric, index) => ({
				...
				candidateRatioToCurrent:
					currentMetric && metric.body.id !== currentMetric.body.id
						? metric.midpointDistance / currentMetric.midpointDistance
						: null,
				isBestCandidate: index === 0,
				isCurrentAutoTarget: metric.body.id === currentMetric?.body.id,
				meetsSwitchThreshold:
					currentSwitchThreshold !== null &&
					metric.body.id !== currentMetric?.body.id
...
const getManualTargetDecision = (
	simulationState: SimulationState,
	selectedIndex: number,
): AssistTargetDecision => {
	const normalizedIndex =
		((selectedIndex % simulationState.bodies.length) +
			simulationState.bodies.length) %
		simulationState.bodies.length;
	const target =
		simulationState.bodies[normalizedIndex] ?? simulationState.bodies[0];
	...
	return {
		debug: {
			...
			bodies: simulationState.bodies.map((body, index) => ({
				...
				midpointDistance: length(
					sub(body.position, simulationState.spacecraft.position),
				),
			})),
...
export const getAssistTargetDecisionForState = (
	simulationState: SimulationState,
	selection: AssistTargetSelection,
): AssistTargetDecision => {
...
export const getAssistTargetForState = (
	simulationState: SimulationState,
	selection: AssistTargetSelection,
): Body => getAssistTargetDecisionForState(simulationState, selection).target;
