...
export const getAutopilotTurnForHeading = (
...
export const getCaptureMetricsForState = (
	simulationState: SimulationState,
	target: Body,
): CaptureMetrics => {
	const relativePosition = sub(
		simulationState.spacecraft.position,
		target.position,
	);
	const relativeVelocity = sub(
		simulationState.spacecraft.velocity,
		target.velocity,
	);
	const distance = length(relativePosition);
	const relativeSpeed = length(relativeVelocity);
	const roughAssistRange =
		target.radius + (target.id === "moon" ? 80_000_000 : 30_000_000);
	const circularSpeed = Math.sqrt(
		(G * target.mass) / Math.max(distance, target.radius),
	);
...
export const shouldCaptureBurnForMetrics = (metrics: CaptureMetrics) =>
	metrics.insideRange &&
	metrics.relativeSpeed > metrics.circularSpeed * captureBurnSpeedRatio;

export const getCircularizePlanForState = (
	simulationState: SimulationState,
	target: Body,
): CircularizePlan => {
	const relativePosition = sub(
...
export const shouldCircularizeBurn = (
	metrics: CaptureMetrics,
	plan: CircularizePlan,
...
export const getAssistPredictionControlsForState = (
	...
	if (assistMode === "capture") {
		...
		return {
			...
			reverse: 0,
			strafe: 0,
			turn: getAutopilotTurnForHeading(
				simulationState.spacecraft.heading,
				desiredHeading,
