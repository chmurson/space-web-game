...
export const createRendererProfiler = (renderer: THREE.WebGLRenderer) => {
  ...
  const updatePendingGpuQueries = () => {
    ...
    while (pendingGpuQueries.length > 0) {
      ...
      smoothedGpuMs =
        smoothedGpuMs === null ? elapsedNanoseconds / 1_000_000 : THREE.MathUtils.lerp(smoothedGpuMs, elapsedNanoseconds / 1_000_000, 0.2);
  ...
  return {
    ...
    render: (scene: THREE.Scene, camera: THREE.Camera, performanceDebugEnabled: boolean) => {
